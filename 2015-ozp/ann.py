...
class NeuralNetLearner(Orange.classification.Learner):
    ...
    def h(self, a, thetas, scale=False):
    ...
    def J(self, thetas):
        err = np.sum((self.h(self.X, thetas) - self.y) ** 2) / 2
    ...
    def fit(self, X, y, W=None):
        ...
        if self.k > 2:
            self.y = np.array([self.y == i for i in range(self.k)])
            self.y = self.y.astype(np.float32).T
    ...
    def test(self, data):
        ...
        print("J", self.J(thetas))
        g0 = self.grad_approx(thetas)
        g1 = self.backprop(thetas)
...
class NeuralNetClassifier(Orange.classification.Model):
    ...
    def predict(self, X):
        if self.k > 2:
            ...
            # y_hat = y_hat / y_hat.sum(axis=1).reshape((y_hat.shape[0], 1))
            return y_hat
        else:
...
data = Orange.data.Table("otto-10000.tab")

Orange.evaluation.sample(data, 10)
...
ann = NeuralNetLearner((n, n, k), lambda_=0.5)

# model = ann(data)
# print(model(data[:5], 1))
# ann.test(data)
...
# sm = Orange.classification.SoftmaxRegressionLearner(lambda_=20)
# mc = helper.MulticlassLearner(rf)

res = Orange.evaluation.CrossValidation(data, [rf], k=3)
# scores = Orange.evaluation.AUC(res)
scores = helper.LogLoss(res)
print(scores)